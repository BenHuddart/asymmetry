...
class TrendPreviewCanvas(QWidget):
    ...
    def _draw_suggestion(
        ...
        """Draw one BED "next point" overlay band via the shared helper.

        Delegates to
        :func:`asymmetry.gui.widgets.suggestion_overlay.draw_suggestion_overlay`
        so this canvas and the Knight-shift window render the band identically
        (see :meth:`set_suggestion` / :meth:`set_discrimination`).
        """
        draw_suggestion_overlay(ax, overlay, colour, label)
