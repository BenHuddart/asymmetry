...
(see Brandt PRB 37, 2349, 1988). The second moment of :math:`P(B)`
...
:math:`\sigma(T)` point by point — see :doc:`/user_guide/fitting`
and :doc:`/user_guide/grouped_time_domain_fitting`.
...
:doc:`/user_guide/composite_models`), fit, and record the Gaussian
...
muSR literature for MgB₂ (Niedermayer *et al.* PRB 65, 094512,
...
- **Treating σ(T) as the only observable.** σ → λ is only exact in
  ...
  corrections; see Sonier RMP 72, 769 (2000) Appendix B.
...
- Sonier, Brewer & Kiefl, *Rev. Mod. Phys.* 72, 769 (2000) — the
...
- :doc:`/user_guide/composite_models` — building the
  ``Oscillatory + Gaussian + Constant`` time-domain composite.
- :doc:`/user_guide/grouped_time_domain_fitting` — when per-detector
  second-moment analysis is needed.
- :doc:`/user_guide/fourier_analysis` — for inspecting the
  :math:`P(B)` line shape directly.
- :doc:`/user_guide/parameter_trending` — for the
  σ → λ → gap-model pipeline.
- :doc:`/user_guide/sc_penetration_depth` — the parametric-model
  reference page.
