...
|---|---|
| data-reduction-parity | TF calibration runs for the diamagnetic alpha estimator vs `estimate_alpha` baseline; LF/ZF relaxing runs (EMU LF series 10–100 G @ 350 K, already used for the integral study) for the "General" alpha method, which the ΣF/ΣB estimator cannot handle; pulsed ISIS runs for tail-fit background (no pre-t0 region); photo-μSR silicon runs 103277–103298 (multi-period) for subset→R/G mapping; long-time relaxation runs for variable/constant-error binning. |
