...
| Fitting dpsep | user can fit dpsep | dpsep **fixed by default; optionally located by a coarse→fine scan** | **Stated divergence (implementation finding), now resolved.** The second-pulse onset gate `t > dpsep/2` is non-smooth in dpsep, so gradient (migrad) fitting of dpsep is unreliable — it converges only from a seed on the correct side of the true value. dpsep therefore defaults to the fixed instrument value; with the optional **fit** toggle it is instead located by a coarse→fine 1-D grid scan over the parameter's bounds (migrad refines the other parameters at each grid point; the best χ² wins), which recovers dpsep from an arbitrary in-range start. With dpsep at its true value the model fits cleanly (χ²ᵣ ≈ 1); a wrong dpsep visibly degrades the fit. Asymmetry diverges from WiMDA's direct gradient fit only in the refinement *method* (scan vs migrad), not the capability. |
